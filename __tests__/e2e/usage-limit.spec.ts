 ...
 * Verifies that an unauthenticated user hitting the API 4× gets a 429 on the 4th request.
 * Uses the API directly so the test is fast and deterministic.
 */

test.describe("Anonymous usage limit", () => {
    test("returns 429 after 3 uses for an anonymous user", async ({ request }) => {
        ...
        // First 3 requests should be allowed (may succeed or fail with 502 if no providers,
        // but must NOT be 429)
        for (let i = 0; i < 3; i++) {
